...
import {
  ...
  doFetchTrackingList,
  reducer,
} from 'src/features/home/redux/fetchTrackingList';

describe('home/redux/fetchTrackingList', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('correct action by fetchUserData', () => {
    expect(fetchTrackingList()).to.have.property('type', HOME_FETCH_TRACKING_LIST_BEGIN);
  });

  it('returns correct action by dismissFetchTrackingListError', () => {
    expect(dismissFetchTrackingListError()).to.have.property('type', HOME_FETCH_TRACKING_LIST_DISMISS_ERROR);
  });

  // saga tests
  const generator = doFetchTrackingList();
  ...
  it('dispatches HOME_FETCH_USER_DATA_FAILURE action when failed', () => {
    const generatorForError = doFetchTrackingList();
  ...
  // reducer tests
  it('handles action type HOME_FETCH_TRACKING_LIST_BEGIN correctly', () => {
  ...
  it('handles action type HOME_FETCH_TRACKING_LIST_SUCCESS correctly', () => {
    const prevState = { fetchUserDataPending: true };
    const state = reducer(
      prevState,
      { type: HOME_FETCH_TRACKING_LIST_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.fetchUserDataPending).to.be.false;
  });

  it('handles action type HOME_FETCH_TRACKING_LIST_FAILURE correctly', () => {
    const prevState = { fetchUserDataPending: true };
    const state = reducer(
      prevState,
      { type: HOME_FETCH_TRACKING_LIST_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.fetchUserDataPending).to.be.false;
  ...
  it('handles action type HOME_FETCH_TRACKING_LIST_DISMISS_ERROR correctly', () => {
